...
Try to summarize the results (bootstrap SE)
```{r}
boot_results |> 
  group_by(term) |> 
  summarize(
...
Fit a regression of `price` on other variables and look sat the residuals 
```{r}
airbnb_fit = 
  nyc_airbnb |> 
...
Run a bootstrap on whole thing to get estimates for the effect of `stars` on `price`
```{r}
manhattan_df = 
  nyc_airbnb |> 
